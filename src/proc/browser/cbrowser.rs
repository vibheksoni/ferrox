...
pub fn find_path() -> Option<String> {
    let paths = [
        sprotect!("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\chrome.exe"),
        sprotect!("SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\chrome.exe"),
    ];
